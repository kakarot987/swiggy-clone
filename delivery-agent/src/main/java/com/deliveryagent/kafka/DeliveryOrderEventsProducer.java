package com.deliveryagent.kafka;
...
import com.deliveryagent.config.ConsumerType;
import com.deliveryagent.domain.Order;


@Component
public class DeliveryOrderEventsProducer {
	
	private static final Logger logger = LoggerFactory.getLogger(DeliveryOrderEventsProducer.class);
	@Autowired
	...
	public ListenableFuture<SendResult<String, String>> sendOrderEvent(Order order, ConsumerType consumerType) throws JsonProcessingException {
		...
		ListenableFuture<SendResult<String, String>> listenableFuture = (ListenableFuture<SendResult<String, String>>) kafkaTemplate.send(producerRecord);
